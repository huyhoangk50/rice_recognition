...
function [accuracy, recall] = checkPerform_spectralFeat(datafile)
...
for i=1:nTrial
    ...
    ntree  = 500;
    modelRF = classRF_train(traindata(:,1:256),trainlabel,ntree);

    %%% test model using valid dataset
    predictlabel = classRF_predict(validdata(:,1:256),modelRF);
