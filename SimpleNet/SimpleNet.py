...
for g in range(0, len(groups)):
    ...
    for count in range(0,5):    
        ...
        for i in range(0, nClasses):
            ...
            fileNames = '../Result/Group_v3' + str(g) + '/time_' + str(count) + 'spatial' +'_sp_' + group[i] + '.csv'
            if not os.path.exists('../Result/Group_v3' + str(g)):
                os.makedirs('../Result/Group_v3' + str(g))
            with open(fileNames, 'wb') as dictFile:
                # print '--------------------------------------------------------------------------'
                writer = csv.writer(dictFile)
                writer.writerow(FIELD_NAMES)
                writer.writerows(spatialResult)
            fileNames = '../Result/Group_v3' + str(g) + '/time_' + str(count) + 'spec' +'_sp_' + group[i] + '.csv'
            with open(fileNames, 'wb') as dictFile:
                writer = csv.writer(dictFile)
                writer.writerow(FIELD_NAMES)
